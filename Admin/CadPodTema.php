...
<label>Selecione seu Tema</label>
<select class="form-control" name="tema">
<?php
$tema = obterTema();
foreach ($tema as $tem) {
  echo "<option value=" . $tem['id'] . ">" . $tem['descricao'] . "</option>";
}
?>
</select><br /><br />
<label>Selecione Podcast</label>
...
<input type="submit" class="btn btn-primary mb-2 center-block btn-lg" name="Enviar" value="Enviar"/><br /><br />
  ...
  if(isset($_POST["Enviar"])){
    ...
    if(empty($podcast) || empty($tema)){
      ...
      $result = mysqli_query($conn, "INSERT INTO podcast_tema VALUES (DEFAULT, $podcast, $tema)");
